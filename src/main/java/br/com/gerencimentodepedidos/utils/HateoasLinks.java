...
public class HateoasLinks {
    public void links(Object dto) {
        if (dto instanceof ProductDTO dto1) {
            ...
            dto2.add(linkTo(methodOn(OrderController.class).findById(dto2.getId())).withRel("findProductById").withType("GET").withTitle("Search for order"));
            dto2.add(linkTo(methodOn(OrderController.class).findAll()).withRel("findAllProducts").withType("GET").withTitle("Search for all orders"));
            dto2.add(linkTo(methodOn(OrderController.class).create(dto2)).withRel("createProduct").withType("POST").withTitle("Create order"));
            dto2.add(linkTo(methodOn(OrderController.class).delete(dto2.getId())).withRel("deleteProductById").withType("DELETE").withTitle("Delete order"));
        } else if (dto instanceof OrderItemDTO dtoItem) {
            Long orderId = (dtoItem.getOrderDTO() != null) ? dtoItem.getOrderDTO().getId() : null;

            if (orderId != null) {
                dtoItem.add(linkTo(methodOn(OrderItemController.class).create(orderId, dtoItem)).withRel("createProduct").withType("POST").withTitle("Create item"));
            }
            dtoItem.add(linkTo(methodOn(OrderItemController.class).findById(dtoItem.getId())).withRel("findProductById").withType("GET").withTitle("Search for item"));
            dtoItem.add(linkTo(methodOn(OrderItemController.class).findAll()).withRel("findAllProducts").withType("GET").withTitle("Search for all items"));
            dtoItem.add(linkTo(methodOn(OrderItemController.class).update(dtoItem.getId(), dtoItem)).withRel("updateProductById").withType("PUT").withTitle("Update item"));
            dtoItem.add(linkTo(methodOn(OrderItemController.class).delete(dtoItem.getId())).withRel("deleteProductById").withType("DELETE").withTitle("Delete item"));
        }
    }
}
